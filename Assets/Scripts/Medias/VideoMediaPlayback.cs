...
public class VideoMediaPlayback : MonoBehaviour
{
    ...
    void Update()
    {
        if (isPlaying)
        {
            ...
            if (System.DateTime.Now.Ticks >= frameNextAt)
            {
                frameNextAt += (System.TimeSpan.TicksPerSecond / EvidenceCapture.videoFramesPerSecond);
                currentFrame++;
                if (currentFrame>= EvidenceCapture.videoFramesPerSecond*15)
                {
                    isPlaying = false;
                    currentFrame = 0;
                }
                ...
                tex.LoadRawTextureData(myEvidence.GetVideoFrame(currentFrame));
                tex.Apply();
    ...
    private void Reset()
    {
        ...
        //TEXTURE SET
        Texture2D tex = new Texture2D(myEvidence.frameWidth, myEvidence.frameHeight, Controller.videoFormat, false);
        tex.LoadRawTextureData(myEvidence.GetVideoFrame(0));
        tex.Apply();
        myImage.sprite = Sprite.Create(tex, new Rect(0.0f, 0.0f, myEvidence.frameWidth, myEvidence.frameHeight), new Vector2(0.5f, 0.5f));
        Vector2 tempScale = myImage.transform.localScale;
        if (myEvidence.frameWidth < myEvidence.frameHeight)
